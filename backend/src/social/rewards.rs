...
pub async fn daily_status(pool: &SqlitePool, user_id: &str) -> Result<DailyStatusResponse, SocialError> {
    let today = Utc::now().format("%Y-%m-%d").to_string();
    let claimed_today: (i64,) = sqlx::query_as("SELECT COUNT(*) FROM daily_rewards WHERE user_id = ? AND claim_date = ?")
    ...
    let current_streak_day = match last {
        Some((day, date)) if date == today => day,
        Some((day, date)) if date == yesterday => (day % 7) + 1, // continues tomorrow's slot
        _ => 1, // streak broken or first time
    };
...
pub async fn claim_daily(pool: &SqlitePool, user_id: &str) -> Result<DailyClaimResponse, SocialError> {
    let status = daily_status(pool, user_id).await?;
